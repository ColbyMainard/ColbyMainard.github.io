...
- **Acronyms get `<abbr title="...">` on first use in a section**, with the term spelled out in the `title`. Twenty-nine are marked up across six pages: `privacy.html` (9), `index.html` (8), `tech_resources.html` (8), `hobbies.html` (2), `guides.html` (1), `tech_takes.html` (1). Nothing in the SCSS styles `abbr`; the browser default is intentional. Match the pattern when you introduce a new acronym.
...
2. **`sitemap.xml` omits `privacy.html`.** Its five `<loc>` entries are `/`, `tech_takes.html`, `tech_resources.html`, `guides.html`, and `hobbies.html`. `assets/html/privacy.html` is missing even though `llms.txt` lists it and `robots.txt` does not disallow it. The add-a-page checklist above says to register every page, so treat this as drift to fix, not as a deliberate exclusion.
