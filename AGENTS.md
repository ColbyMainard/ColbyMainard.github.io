...
1. **`path_helpers.js` is loaded by no page.** `grep -rn 'path_helpers' --include=*.html .` returns nothing, yet the file is precached at `service-worker.js:32` and two scripts depend on it. Consequences, all silent: the service worker is **never registered**, `<link rel="manifest">` is **never injected** (so the site has no offline support and is not installable), and the consent banner ships without its privacy-policy link. `service_worker_register.js` returns at line 23 *before* reaching the `console.warn` added to surface exactly this class of regression. Fix: add the tag above `cookie_consent.js` on all six non-404 pages — `./assets/js/path_helpers.js` at repo root, `../js/path_helpers.js` under `assets/html/` — bump `CACHE_VERSION`, then **verify by observation** over `http://localhost`: `window.PathHelpers` defined, a `link[rel="manifest"]` in the DOM, a registered worker in DevTools, and (after clearing `localStorage` key `cm-cookie-consent`) a banner containing the privacy link.
2. **`guides.html` has no Atom `<link rel="alternate">`,** though `feed.xml` carries seven guide entries mirroring its `TechArticle` blocks. Half the feed is undiscoverable from the page it describes.
3. **A stale `.takesIndex` comment survives the feature's removal.** The dated index is now gone from every place it was implemented: the markup, the `.takesIndex` style block in `tech_takes.scss` and `default.css`, the `@include animationGate("#introSectionDiv", "> .takesIndex")` in the SCSS, and the animation step in `tech_takes_animations.js`. What is left is the doc comment above `animateIntro` at `tech_takes_animations.js:37-50`, which still justifies writing the function locally "because the `.takesIndex` `<nav>` carries its own animation gate (see the comment at `tech_takes.scss:145`)" — pointing at a gate and a comment that no longer exist. Rewrite or delete it. While you are there, reassess whether `animateIntro` still needs to be local at all now that its only stated reason is gone, or whether it can call `AnimationHelpers.animateIntro` like the other pages.
4. **`.relatedStances` is reserved, not dead.** It survives in the word-count exclusion at `reading_engagement.js:36`, anticipating a per-take related-links component that has not been built. The SCSS comment that explained the name was removed along with the `.takesIndex` block, so this entry is now its only documentation. Keep the name if you build it — the reading-time calculation already excludes it. Note that the shared `animationGate` on the take sections matches `> h1, > h2, > h3, > h4, > p, > ul, > dl, > .tableScroll, > a` and does **not** match `> nav`, so a future `.relatedStances` nav will need its own gate and animation step, or none at all.
5. **~22 MB of unoptimized imagery.** The five `photographyHobby/` originals (3.5–5.0 MB each) and `miscellaneous/DEFCON33.jpeg` (2.1 MB) are camera originals served unmodified. `DEFCON33.jpeg` is the LCP element on `hobbies.html` **and** carries `loading="lazy"` (`hobbies.html:144`), which delays the paint it is measured on. This is the constraint that forced the precache exception below; shrinking them removes it rather than working around it.
6. **Entity graph is not consolidated.** `index.html` declares a canonical `Person` at `"@id": "https://colbymainard.github.io/#person"`, but no subpage references it — `grep -rn '#person' --include=*.html assets/html/` returns nothing. Instead, roughly forty-four author/publisher nodes across `guides.html` (23), `tech_takes.html` (16), and `hobbies.html` (5) re-declare Colby as a fresh anonymous `Person`. Replace each with `{"@id": "https://colbymainard.github.io/#person"}`. Leave `tech_resources.html` alone — its many `Person` nodes are book and course authors, genuinely different people.
