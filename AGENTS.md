...
- `section_permalinks.js` (`tech_takes.html` and `guides.html`) appends a "Copy link" button to each section heading. It is **load-order-critical in the same way `path_helpers.js` is, but for a different reason**: it must sit **below `reading_engagement.js`** in the `<head>`. Both wait for `DOMContentLoaded` and handlers fire in registration order, so this ordering is what keeps the button labels out of the per-section word counts. It builds nothing outside `http(s)` — from `file://` there is no shareable URL and no Clipboard API, so no button is rendered rather than a control that copies a local path. It resolves each heading's anchor to the **outermost** ancestor id below `<main>` (`#VibeCodingScourgeDiv`, not `#VibeCodingScourge`) so the copied URL matches what `feed.xml`, the `Article` JSON-LD, and the "Cite this guide:" lines already publish. It never moves focus, and it announces through the static `#sectionPermalinkStatus` `role="status"` paragraph in each page's `<main>` (static for the same reason `#photoGalleryStatus` is).
