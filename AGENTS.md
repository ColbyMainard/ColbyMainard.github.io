...
- **Shared deferred scripts loaded on every page:** `cookie_consent.js`, `navbar.js`, `back_to_top.js`, `service_worker_register.js` — and on animated pages, `animation_helpers.js` + the page's own `*_animations.js`, plus occasional helpers (e.g. `reading_engagement.js` reading-time and progress bar, loaded by `tech_takes.html` and `guides.html`).
