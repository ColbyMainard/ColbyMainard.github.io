...
| `assets/markdown/` | Progress and design notes. Dated strategy/audit reports are also written here as `<topic>-YYYY-MM-DD.md`, but they are disposable — the 2026-08-04 set was deleted on 2026-08-05 and the 2026-08-26 set on 2026-08-26, in both cases once acted on. As of 2026-08-27 the directory holds only `animations_report.md`. Check what is actually on disk; do not assume an audit exists, and do not assume one does not. |
...
- **Shared structure lives in mixins, not in the partials.** `default.scss` defines ten: `sectionShell`, `headingRamp`, `bodyText`, `unorderedListText`, `orderedListText`, `blockQuote`, `proseLinks`, `dataTable`, `sectionPermalink`, and `animationGate`. List text is split into two mixins, not one `listText`. Each partial `@include`s the ones it needs with its own palette; all seven include `sectionShell`, `headingRamp`, `bodyText`, `unorderedListText`, and `proseLinks`, six of seven include `animationGate`, five include `dataTable`, four include `blockQuote`, three include `sectionPermalink`, and exactly one includes `orderedListText`. The `animationGate` exception is `privacy_policy.scss`: `privacy.html` deliberately loads no AnimeJS or animation scripts, so adding the gate there would set the page to `opacity: 0` with nothing left to reveal it. Do not "fix" that omission. The single `orderedListText` call is `tech_takes.scss:154`, inside the `#VibeCodingScourge` block, which covers that page's only `<ol>` contexts. The three `sectionPermalink` calls are `tech_takes.scss`, `guides.scss`, and `tech_resources.scss`, the three pages that load `section_permalinks.js`; the mixin styles the centered `.sectionPermalinkWrap` block that closes each section card and the `.sectionPermalink` button inside it. `sectionShell` also carries the site's only `@media (forced-colors: active)` section rule: forced colors discards the card's background tint, so the mixin restores the boundary with a `1px solid CanvasText` border for all seven pages at once (`header` and `footer` carry the matching band rules). Put a structural change (border style, spacing, a newly animated element type) in the mixin — hand-copying it into six partials is what the mixins exist to prevent. Only page-specific rules (the hobbies iframe and photo gallery, the tech-takes glossary, per-cell link colors) belong at the call site.
...
- `section_permalinks.js` (`tech_takes.html`, `guides.html`, and `tech_resources.html`) appends one "Copy link" button per section card, as the card's **last child**, centered by `.sectionPermalinkWrap`. It drives off `h2` only, so a card gets one control rather than one per subheading; an earlier version placed the button inline inside the heading and was moved on 2026-09-06. It is **load-order-critical in the same way `path_helpers.js` is, but for a different reason**: on the two pages that also load `reading_engagement.js` it must sit **below** it in the `<head>`. Both wait for `DOMContentLoaded` and handlers fire in registration order, so this ordering is what keeps the button labels out of the per-section word counts. `tech_resources.html` loads no reading-time script and so carries no such constraint, which the comment above its tag says explicitly. The buttons are built on **every** origin, including `file://`; from a non-`http(s)` origin the button copies the canonical published URL for that section (`https://colbymainard.github.io/...`) rather than a `file:///C:/...` path nobody else can open, and the copy runs through the `execCommand` fallback because the Clipboard API needs a secure context. Do not restore the old guard that rendered nothing locally. It resolves each heading's anchor to the **outermost** ancestor id below `<main>` (`#VibeCodingScourgeDiv`, not `#VibeCodingScourge`) so the copied URL matches what `feed.xml`, the `Article` JSON-LD, and each page's own section menu already publish. (`guides.html` printed a "Cite this guide:" line per section until commit 075515b on 2026-09-06, which removed all seven. No page prints one today, and this paragraph described them as current until 2026-09-12.) It never moves focus, and it announces through the static `#sectionPermalinkStatus` `role="status"` paragraph in each page's `<main>` (static for the same reason `#photoGalleryStatus` is).
...
- Existing patterns to keep: skip links are intentionally **screen-reader-only** (`<a href="#main" class="visually-hidden">Skip to main content</a>`, one per page on all seven, always hidden rather than focus-reveal); data tables carry a `visually-hidden` `<caption>` and `scope`-ed `<th>`; decorative SVGs use `aria-hidden="true"`; **new** inline SVGs should use `role="img"` + `<title>`.
- **Acronyms get `<abbr title="...">` on first use in a section**, with the term spelled out in the `title`. Seventy-two are marked up across six pages: `guides.html` (23), `tech_takes.html` (22), `privacy.html` (9), `index.html` (8), `tech_resources.html` (8), `hobbies.html` (2). One SCSS rule styles them: `abbr[title] { text-decoration: underline dotted; cursor: help; }` in `default.scss`, added on 2026-09-03. It sets no color on purpose, since `text-decoration` draws in `currentColor` and inherits each section's already-AA tone. This overturned the earlier "nothing styles `abbr`" convention as a deliberate decision (roadmap conflict C1) — **do not revert it as drift**. It is half a fix: the `title` is still unreachable by keyboard (`abbr` is not focusable) and by touch, so spell a term out in prose where that matters. Match the pattern when you introduce a new acronym.
...
- If you **added** a stance or guide, two prose lists name every one of them and go stale without any error: the Technical Stances / Guides lines in `llms.txt` (newest first) and the matching lists in `README.md` (page order). Neither is generated, and moving the `llms.txt` date does not check them.
...
Confirmed defects as of **2026-08-27**. These are bugs, **not** conventions to preserve — do not "match the existing pattern" on any of them. The dated audit reports that first described them have since been deleted from `assets/markdown/` under the disposable-reports rule, so **this list is the record** — confirm each against the source files rather than looking for a report.

1. **~22 MB of unoptimized imagery.** The five `photographyHobby/` originals (3.5–5.0 MB each) and `miscellaneous/DEFCON33.jpeg` (2.1 MB) are camera originals served unmodified. `DEFCON33.jpeg` is the LCP element on `hobbies.html` **and** carries `loading="lazy"` (`hobbies.html:149`), which delays the paint it is measured on. This is the constraint that forced the precache exception below; shrinking them removes it rather than working around it.

### Settled, not a gap: `sitemap.xml` omits `privacy.html`

Resolved on **2026-09-01**, recorded here because this list previously called it drift and a reader acting on that would break the site. **The sitemap is correct. Do not add `assets/html/privacy.html` to it.** That page carries `<meta name="robots" content="noindex, follow">` (`privacy.html:47`), and a sitemap is a list of URLs the site asks to have indexed. Listing a `noindex` URL sends contradictory instructions and produces a "Submitted URL marked 'noindex'" error in Search Console. `404.html` is excluded for the same reason and has never been flagged as drift. The five `<loc>` entries (`/`, `tech_takes.html`, `tech_resources.html`, `guides.html`, `hobbies.html`) are the complete set of indexable pages. The add-a-page checklist above applies to indexable pages; a `noindex` page belongs in `llms.txt` and not in `sitemap.xml`.

### Closed on 2026-08-27

- **`llms.txt` and `README.md` now list all eight Technical Stances.** Both had been left at seven when "Vibe Coding is a Scourge to the Internet" was added, and `llms.txt` had its `**Last updated**` line moved to 2026-08-27 in that same commit, so the staleness was invisible from the date alone. When you add a stance, re-read the page's `Article` headlines and update the prose summaries in both files, not just the date. `CACHE_VERSION` was bumped to `v72` because `llms.txt` is precached and served cache-first.
...
- **`guides.html` now links the Atom feed** from its `<head>`, so all fifteen entries (eight stances plus seven guides, as of 2026-08-27) are discoverable from both pages the feed covers. See SEO & structured data.
