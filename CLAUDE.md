...
- Each page loads a shared set of deferred scripts — `cookie_consent.js`, `navbar.js`, `back_to_top.js`, `clipboard.js`, `service_worker_register.js` — and animated pages add `animation_helpers.js` plus their own `<page>_animations.js` (and occasionally page-specific helpers such as `tech_takes_engagement.js`). Two exceptions to preserve: `privacy.html` loads no AnimeJS/animation scripts, and `404.html` omits `cookie_consent.js` and `service_worker_register.js` and uses root-absolute paths, because GitHub Pages serves its content at arbitrary missing URLs where those scripts' relative-path logic breaks.
