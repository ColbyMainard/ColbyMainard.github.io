...
- Each page loads a shared set of deferred scripts — `cookie_consent.js`, `navbar.js`, `back_to_top.js`, `service_worker_register.js` — and animated pages add `animation_helpers.js` plus their own `<page>_animations.js` (and occasionally shared helpers such as `reading_engagement.js`, loaded by `tech_takes.html` and `guides.html`). Two exceptions to preserve: `privacy.html` loads no AnimeJS/animation scripts, and `404.html` omits `cookie_consent.js` and `service_worker_register.js`, because GitHub Pages serves its content at arbitrary missing URLs where those scripts' relative-path logic breaks. `404.html` otherwise uses relative paths like every other page — a deliberate tradeoff that keeps it working from `file://` and at top-level missing URLs, accepting that it renders unstyled at nested missing URLs (e.g. `/dir/typo`).
