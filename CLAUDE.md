...
- Abbreviations and acronyms are marked up with `<abbr title="...">` on first use in a section, spelling the term out in the `title`. Twenty-nine such elements exist across six pages: `privacy.html` (9), `index.html` (8), `tech_resources.html` (8), `hobbies.html` (2), `guides.html` (1), `tech_takes.html` (1). No CSS targets `abbr`; the browser default is what ships. Follow the pattern when introducing a new acronym.
- One hand-maintained Atom feed (`feed.xml`) covers both the Technical Stances page (`tech_takes.html`) and the Guides page (`guides.html`); it carries one entry per stance and one per guide. Both pages link it from their head via `<link rel="alternate" type="application/atom+xml">`, and `tech_takes.html` additionally carries a visible "Subscribe (Atom)" link in the body that `guides.html` does not. The two head links share one `feed.xml` but carry different `title` text naming what that page's reader gets; that is deliberate. No other page links the feed.
...
- `default.scss` also holds the shared section mixins every partial calls with its own palette: `sectionShell`, `headingRamp`, `bodyText`, `unorderedListText`, `orderedListText`, `blockQuote`, `proseLinks`, `dataTable`, and `animationGate`. Unordered and ordered list text are two separate mixins, not one `listText`; all seven partials include the unordered one and only two include the ordered one. Structural changes (a border style, a newly animated element type) belong in the mixin, not copied into each partial; only genuinely page-specific rules stay at the call site.
...
- Every file in `assets/js/` should be referenced by at least one page. Keep it that way: a script that no page loads should be deleted rather than left precached (the legacy `current_time.js` was removed on 2026-07-23 for exactly this reason). As of 2026-08-27 all fifteen files are referenced and all fifteen are listed in `PRECACHE_URLS`, so an unreferenced script is a new problem rather than a known one.
...
- **`sitemap.xml` omits `privacy.html`.** It lists five `<loc>` entries (`/`, `tech_takes.html`, `tech_resources.html`, `guides.html`, `hobbies.html`) and skips `assets/html/privacy.html`, which `llms.txt` does list and `robots.txt` does not disallow. The add-a-page checklist says every page gets registered, so this is drift, not an exclusion policy.
